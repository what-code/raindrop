...
/**
 * 改进{@link MetaSpout}的count down的阻塞方式
 * 
 * @author Jacky Liu, LiBin Jin
 * @date 2011-11-8
 * 
 */
public class BlockedMetaSpout implements IRichSpout {
    ...
    static final Log log = LogFactory.getLog(BlockedMetaSpout.class);
    ...
    public BlockedMetaSpout(final MetaClientConfig metaClientConfig, final ConsumerConfig consumerConfig, final Scheme scheme) {
        super();
    ...
    private void setUpMeta(final String topic, final Integer maxSize) throws MetaClientException {
        ...
        this.messageConsumer.subscribe(topic, maxSize, new MessageListener() {
            ...
            public void recieveMessages(final Message message) {
                ...
                //BlockedMetaSpout.this.id2wrapperMap.put(message.getId(), wrapper);
                try {
					BlockedMetaSpout.this.messageQueue.transfer(wrapper);
	                log.info(new StringBuilder("InQueue : ").append(message.getId()).toString());
    ...
    public void nextTuple() {
        if (this.messageConsumer != null) {
            try {
            	...
            	final MetaMessageWrapper wrapper = this.messageQueue.take();
                if (wrapper == null) {
                    return;
                }
